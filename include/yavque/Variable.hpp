...
public:
	...
	void change_to(const Variable& rhs) { p_ = rhs.p_; }

	bool operator==(const Variable& rhs) const { return p_ == rhs.p_; }
	bool operator!=(const Variable& rhs) const { return p_ != rhs.p_; }

	[[nodiscard]] std::string name() const { return p_->name(); }

	[[nodiscard]] std::string desc() const { return p_->desc(); }

	[[nodiscard]] double value() const { return p_->value(); }

	void set_value(double new_value) { p_->set_value(new_value); }
